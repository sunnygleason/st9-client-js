...
St9Client.prototype = {
  ...
  _get : function(path, cc) {
    return this._do_request('GET', path, null, cc);
  },
  _post : function(path, body, cc) {
    return this._do_request('POST', path, JSON.stringify(body), cc);
  },
  _put : function(path, body, cc) {
    return this._do_request('PUT', path, JSON.stringify(body), cc);
  },
  _delete : function(path, cc) {
    return this._do_request('DELETE', path, null, cc);
  },
  admin_nuke : function(preserveSchema, cc) {
    return this._post("/1.0/nuke?preserveSchema=" + !!preserveSchema, null, cc);
  },
  admin_export : function(cc) {
    return this._get("/1.0/x", cc);
  },
  admin_import : function(json_rows, cc) {
    return this._post("/1.0/x", json_rows, cc);
  },
  schema_get : function(type, cc) {
    return this._get("/1.0/s/" + type, cc);
  },
  schema_create : function(type, body, cc) {
    return this._post("/1.0/s/" + type, body, cc);
  },
  schema_update : function(type, body, cc) {
    return this._put("/1.0/s/" + type, body, cc);
  },
  schema_delete : function(type, cc) {
    return this._delete("/1.0/s/" + type, cc);
  },
  entity_get : function(id, cc) {
    return this._get("/1.0/e/" + id, cc);
  },
  entity_multiget : function(ids, cc) {
	var q = ids.map(function(x) { return "k=" + x; }).join("&");
	
    return this._get("/1.0/e/multi?" + q, cc);
  },
  entity_create : function(type, body, cc) {
    return this._post("/1.0/e/" + type, body, cc);
  },
  entity_update : function(id, body, cc) {
    return this._put("/1.0/e/" + id, body, cc);
  },
  entity_delete : function(id, cc) {
    return this._delete("/1.0/e/" + id, cc);
  },
  index_get : function(type, index, query, cc) {
    return this._get("/1.0/i/" + type + "." + index + "?q=" + escape(query), cc);
  },
  counter_get : function(type, counter, values, cc) {
    return this._get("/1.0/c/" + type + "." + counter + "/" + values.join("/"), cc);
  }
}
